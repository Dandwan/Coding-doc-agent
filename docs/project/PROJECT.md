...
- 文档生成与版本管理
  ...
  - 每次生成最终文档时，都会稳定写入一段 DocAgent 固定规范，明确项目开发文档位置、维护责任、AGENT_DEVELOPMENT.md 输出位置，以及积极上传要求
  - 若项目启用了积极上传并指定分支，最终文档中会明确要求“每完成一个新功能就提交并上传一次”，并写明目标分支
  - 会话历史若明确要求额外补充“更新项目开发文档”或“执行积极上传”指令，则再追加独立附加指令块
  - 固定规范块和附加指令块都具备可覆盖更新能力，避免同一文档重复追加
...
4. 提交后系统：
   ...
   - 调用 LLM（失败则 fallback）
   - 生成/更新 Agent 开发文档
   - 写入固定规范块（项目开发文档位置/维护责任、Agent 文档输出位置、积极上传要求）
   - 按会话意图补充附加执行指令（更新项目开发文档/积极上传）
   - 生成文档版本快照
5. 用户可查看、对比、恢复历史版本。
6. 会话文件和文档版本持续落盘。
...
- 已提供 pytest 回归测试，覆盖：
  ...
  - 积极上传默认/项目覆盖
  - 完成后新增需求重开核实
  - 文档生成器的固定规范注入与附加指令注入（项目文档路径、积极上传开关与分支场景）
...
- 当每个功能开发完成时，除代码提交与上传外，还应同步更新本文档中的：
    *   项目概述
    *   项目详细介绍
    *   项目架构
    *   已实现的功能清单
